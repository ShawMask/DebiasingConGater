...
def get_optimizers(args, model):
    ...
    bottleneck_no_decay_pgroup = []  # parameter group for non-encoder part, contains biases and LayerNorm params
    bottleneck_decay_pgroup = []  # parameter group for non-encoder part. L2 regularization will be applied
    congater_no_decay_pgroup = []  # parameter group for non-encoder part, contains biases and LayerNorm params
    congater_decay_pgroup = []  # parameter group for non-encoder part. L2 regularization will be applied
    for name, param in model.named_parameters():
        if name.startswith('encoder'):
            ...
            if any(st in name for st in no_decay_str):
                bottleneck_no_decay_pgroup.append(param)
            else:
                bottleneck_decay_pgroup.append(param)
        elif name.startswith('congater'):
            if any(st in name for st in no_decay_str):
                congater_no_decay_pgroup.append(param)
            else:
                congater_decay_pgroup.append(param)
        else:
            if any(st in name for st in no_decay_str):
    ...
    #nonencoder consists of: scorer module, bottleneck, congater and each can get another learning rate
    nonencoder_optim_pgroups = [{'params': nonencoder_no_decay_pgroup, 'weight_decay': 0, 'lr': args.learning_rate},
                                {'params': nonencoder_decay_pgroup, 'weight_decay': args.weight_decay, 'lr': args.learning_rate}
                                ]
    if len(bottleneck_no_decay_pgroup) > 0:
        nonencoder_optim_pgroups.append({'params': bottleneck_no_decay_pgroup, 'weight_decay': 0, 'lr': args.learning_rate_bottleneck})
    if len(bottleneck_decay_pgroup) > 0:
        nonencoder_optim_pgroups.append({'params': bottleneck_decay_pgroup, 'weight_decay': args.weight_decay, 'lr': args.learning_rate_bottleneck})
    if len(congater_no_decay_pgroup) > 0:
        nonencoder_optim_pgroups.append({'params': congater_no_decay_pgroup, 'weight_decay': 0, 'lr': args.learning_rate_congater})
    if len(congater_decay_pgroup) > 0:
        nonencoder_optim_pgroups.append({'params': congater_decay_pgroup, 'weight_decay': args.weight_decay, 'lr': args.learning_rate_congater})
    # keyword arguments here will be the global defaults
    nonencoder_optimizer = optim_class(nonencoder_optim_pgroups, eps=args.adam_epsilon)
