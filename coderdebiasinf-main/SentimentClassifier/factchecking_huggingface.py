...
tokenizer = AutoTokenizer.from_pretrained("tals/albert-base-vitaminc_flagging",use_fast = False)
model = AutoModelForSequenceClassification.from_pretrained("tals/albert-base-vitaminc_flagging")
...
with open(out_file, "w", encoding="utf8") as fw:
    with open(doc_file, "r", encoding="utf8") as fr:
        for line in tqdm(fr, total=total_size):
            ...
            docid = vals[0]
            doctext = vals[1]
            tok = tokenizer(doctext)
            res = sentiment_pipeline(tok)[0]
