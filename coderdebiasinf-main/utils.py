...
def load_adapter(model, checkpoint_path, device='cpu'):
    """
    Loads query encoder weights from a MDSTransformer model checkpoint
    :param model: an initialized encoder model object (typically HuggingFace), already on its intended device
    :param checkpoint_path: MDSTransfomer checkpoint file from which to load encoder weights.
    ...
    # checkpoint = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)  # Load all tensors onto the CPU
    for key, val in list(checkpoint['state_dict'].items()):
        if not('adapters' in key):  # discard parameter
            checkpoint['state_dict'].pop(key)
        # else:  # keep parameter after trimming name
        #     new_key_name = key[len('encoder.'):]
        #     checkpoint['state_dict'][key] = checkpoint['state_dict'].pop(key)
...
def register_record(filepath, timestamp, experiment_name, best_metrics, final_metrics=None, comment='', w = None, stage = None):
    """
    ...
    row_values = [timestamp, experiment_name, comment,w, stage] + list(metrics_values)
    ...
    if not os.path.exists(filepath):  # Create a records file for the first time
        ...
        header = ["Timestamp", "Name", "Comment", "w", "Stage"] + ["Best " + m for m in metrics_names]
        if final_metrics is not None:
...
def save_inference(args, eval_metrics, ranked_df, w = None):
    ...
    filename = f'reranked_w={w}_' + os.path.basename(args.eval_candidates_path)
    ...
    register_record(args.records_file, args.initial_timestamp, args.experiment_name,
                              eval_metrics, comment=args.comment, w = w)