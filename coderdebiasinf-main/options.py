...
def run_parse_args():
    ...
    parser.add_argument('--override', type=str,
                        help="Optional, to be used with `config`. A string in the format of a python dict, with keys the options "
                             "which should be overriden in the configuration .json file, e.g. {'task':'train'}")
    ## Experiment
    ...
    ## Congater parameters
    parser.add_argument('--learning_rate_congater', default=None, type=float,
                        help="Special learning rate for the congater module. By default same as global learning rate.")
    parser.add_argument('--congater_training_method', default='post', type=str,
                        help="Defines if congater is trained consecutively or step by step. Only training_method implemented at this point is 'post'.")
    parser.add_argument('--num_epochs_warmup', default=0, type=str,
                        help="Number of epochs used for warm-up.")
    parser.add_argument('--evaluate_w', default=[0,0.2,0.4,0.6,0.8,1], type=list,
                        help="Values of w that are used in evaluation mode.")
    parser.add_argument('--congater_names', default=None, type=list,
                        help="Protected attributes in the data.")
    parser.add_argument("--congater_position", type=str
                        , default="all", help="The position where the congater layer is added in the transformer model. Values can be all and last.")
    parser.add_argument("--num_gate_layers", type=int, default=1, help="The number of layers in each congater module.")
    parser.add_argument("--gate_squeeze_ratio", type=int, default=4, help="The scaling ration between the congater layers of gating.")
    parser.add_argument("--congater_version", type=int, default=1, help="Version of the congater ttsigmoid function.")
    parser.add_argument("--default_trainable_parameters", type=str,
                        default="all", help="Changes the default parameters of bert during training.")
    
    ## Adapter parameters:
    parser.add_argument('--adapter_names', default=None, type=str,
                        help="Protected attributes in the data.")
    parser.add_argument("--squeeze_ratio", type=int, default=4, help="The scaling ration between the adapter layers.")
    parser.add_argument("--interpolate_position", type=str
                        , default="all", help="The position where the interpolation layer is added in the transformer model. Values can be all and last.")
    parser.add_argument('--load_adapter', dest='load_adapter_path', type=str, default=None,
                        help='Path to pre-trained adapter checkpoint. If specified, the adapter weights will be loaded, BUT NOT the state'
                             'of the optimizer/scheduler.')
...
def check_args(config):
    ...
    if config['learning_rate_congater'] is None:
        config['learning_rate_congater'] = config['learning_rate']
