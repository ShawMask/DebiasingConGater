...
class AdapterModel(MDSTransformer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #add adapter layer and set to training mode
        if self.adapter_names:
            ...
            self.encoder.train_adapter(self.adapter_names,adap_config)
            #freeze training for scoring module as well
            for name, param in self.named_parameters():
                if name.startswith("score_docs"):
    ...
    def forward(self, query_token_ids: torch.Tensor, query_mask: torch.Tensor = None, doc_emb: torch.Tensor = None,
                ...
                bias_regul_coeff: float = None) -> Dict[str, torch.Tensor]:
        r"""
        ...
        # Changed model call here to Adapter _forward method
        encoder_out = self._forward(**x)  # int64 required by torch nn.Embedding
    ...
    def evaluate(self, args, dataloader, stage, bias_regul_coeff, logger, fairrmetric=None):
        ...
        eval_metrics, ranked_df = self.eval_step(args, dataloader, bias_regul_coeff, logger, fairrmetric)

        return eval_metrics, ranked_df


    def eval_step(self, args, dataloader, bias_regul_coeff, logger, fairrmetric=None):
        """
        ...
        with torch.no_grad():
            for batch_data, qids, docids in tqdm(dataloader, desc="Evaluating"):
                ...
                try:
                    out = self.forward(**batch_data, bias_regul_coeff = bias_regul_coeff)
                except RuntimeError:
    ...
    def validate(self, args, val_dataloader, tensorboard_writer, training_stages, bias_regul_coeff, logger, fairrmetric=None):
        ...
        for label_idx, stage in enumerate(training_stages):
            #Adapter training only has one stage
            assert len(training_stages)==1, "Only one stage for adapter training"

            #call evaluation function
            self.eval()
            eval_start_time = time.time()
            val_metrics, ranked_df = self.evaluate(args, val_dataloader, stage, bias_regul_coeff, logger, fairrmetric=fairrmetric)
            ...
            #code determines the best step according to given metric
            ...
            if condition:
                ...
                register_record(rec_filepath, args.initial_timestamp, args.experiment_name, val_metrics, w = None)
    ...
    def _step(
        ...
        for step, (model_inp, _, _) in enumerate(batch_iterator):
            ...
            for idx, stage in enumerate(training_stages):
                ...
                try:
                    output = self.forward(**model_inp, bias_regul_coeff = bias_regul_coeff)  # model output is a dictionary
                except RuntimeError:
                    raise optuna.exceptions.TrialPruned()
                loss = output['loss']
