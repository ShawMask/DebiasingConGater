...
class AdvModel(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.task_head_state_dict = task_head_state_dict
    ...
    def fit(
        ...
        if training_method == "par":
            for epoch in train_iterator:
                ...
                logs_ = result.copy()
                logs_.update(results_protected)
                logs_["epochs"] = epoch

                if self.wandb_logger is not None:
                    self.wandb_logger.log(logs_)
            ...
            for stage in ["task", "debias"]:
                if stage !="task":
                    for nn, param in self.named_parameters():
                        if "task_head" in nn:
                ...
                print(f"Final {stage} result after " + train_str.format(epoch, result_str))
    ...
    def _step(
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for i, (l, loss_fn_prot) in enumerate(zip(labels_protected, loss_fn_protected)):
                ...
                if training_stage == "task":
                    loss_protected = torch.zeros_like(loss_protected)
                loss += loss_protected
